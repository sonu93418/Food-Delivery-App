...
export default function RestaurantDetailScreen({ route, navigation }) {
  ...
  const handleAddToCart = (item) => {
    addToCart({ id: item.id, name: item.name, price: item.price, restaurantName: restaurant.name });
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {/* Restaurant Info */}
        <Animated.View
          style={[styles.infoContainer, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}
        >
          ...
          <View style={[styles.statsRow, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
            <View style={styles.statItem}>
              ...
              <Text style={[styles.statValue, { color: theme.text }]}>{restaurant.rating}</Text>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={[styles.statValue, { color: theme.text }]}>{restaurant.deliveryTime}</Text>
              <Text style={[styles.statLabel, { color: theme.textSecondary }]}>Delivery</Text>
            </View>
            <View style={[styles.statDivider, { backgroundColor: theme.divider }]} />
            <View style={styles.statItem}>
              <Ionicons name="wallet-outline" size={18} color="#4ade80" />
              <Text style={[styles.statValue, { color: theme.text }]}>₹{avgPrice || 'N/A'}</Text>
              <Text style={[styles.statLabel, { color: theme.textSecondary }]}>Avg Price</Text>
            </View>
          </View>
          ...
          <View style={styles.paramsCard}>
            ...
            <Text style={[styles.paramsText, { color: theme.textSecondary }]}>restaurantId: {restaurantId}</Text>
            <Text style={[styles.paramsText, { color: theme.textSecondary }]}>restaurantName: {restaurantName}</Text>
            <Text style={[styles.paramsText, { color: theme.textSecondary }]}>avgPrice: ₹{avgPrice}</Text>
          </View>
        </Animated.View>
            ...
            >
              <View style={styles.menuItemInfo}>
                ...
                <View style={styles.menuItemText}>
                  ...
                  <Text style={[styles.menuItemDesc, { color: theme.textSecondary }]}>{item.description}</Text>
...
const styles = StyleSheet.create({
  ...
  heroOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(26,26,46,0.3)' },
  backButton: {
  ...
  cartHeaderBadgeText: { color: '#fff', fontSize: 10, fontWeight: '700' },
  infoContainer: { padding: 20 },
  restaurantName: { fontSize: 26, fontWeight: '800', marginBottom: 4 },
  cuisineText: { fontSize: 14, marginBottom: 20 },
  statsRow: {
  ...
  paramsCard: {
    ...
    padding: 14, borderWidth: 1, borderColor: 'rgba(255,107,53,0.2)',
  },
  paramsTitle: {
  ...
  menuTitle: { fontSize: 20, fontWeight: '700', marginBottom: 16 },
  menuItem: {
    flexDirection: 'row', justifyContent: 'space-between',
    alignItems: 'center', borderRadius: 14, padding: 14,
    marginBottom: 10, borderWidth: 1,
  },
  menuItemInfo: { flexDirection: 'row', flex: 1, gap: 10 },
  vegIndicator: {
  ...
  menuItemText: { flex: 1 },
  menuItemName: { fontSize: 15, fontWeight: '600', marginBottom: 2 },
  menuItemDesc: { fontSize: 12, marginBottom: 4 },
  menuItemPrice: { fontSize: 15, fontWeight: '700', color: '#FF6B35' },
  addButton: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: 'rgba(255,107,53,0.12)',
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10,
    borderWidth: 1, borderColor: 'rgba(255,107,53,0.25)', gap: 2,
  },
  addButtonText: { color: '#FF6B35', fontSize: 13, fontWeight: '700' },
  floatingCart: {
  ...
  floatingCartLeft: {},
  floatingCartCount: { color: '#fff', fontSize: 14, fontWeight: '600' },
